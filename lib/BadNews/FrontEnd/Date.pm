# $Id: Date.pm 441 2006-12-11 21:32:47Z corrupt $
package BadNews::FrontEnd::Date;
...
sub handle_request {
    ...
    # set this bad boy up using the url...
    $fe->cgi->param(year                =>      $uri[1]);
    $fe->cgi->param(month               =>      $uri[2]);
    $fe->cgi->param(paged               =>      $uri[3]);

    $r->content_type('text/html');
    $r->send_http_header;
    $fe->render_page("date_archived.tt2");
    return OK;
}

package BadNews::EzDate;
use strict;
use Carp;
# use Debug::ShowStuff ':all';

# $Id: EzDate.pm 428 2006-08-30 02:00:57Z corrupt $

# Copyright (c) 2001-2003 by Miko O'Sullivan.
# dumb module that I'm stuck with, credit to the original author is in here,
# consider this a full fledged fork of Date::EzDate (mostly bug fixes that 
# i'm too lazy to submit patches for or even come up with a new release for)

# I don't even want credit for my changes.  I just want this thing to work 
# properly :\
...
sub next_month {
    ...
    # there is no reason that whatever wack number this shit module comes up with
    # should override the original day of the month.  Unless it is greater than the
    # number of days in the month!
    if (($dom > $orgday) && ($orgday < $dim)) {
        $self->{'dayofmonth'} = $orgday;
    }
...
sub compare {
	my ($left, $right) = @_;
	ref($right) or $right = BadNews::EzDate->new($right);
...
use constant WARN_NONE   => BadNews::EzDate::WARN_NONE;
use constant WARN_STDERR => BadNews::EzDate::WARN_STDERR;
use constant WARN_CROAK  => BadNews::EzDate::WARN_CROAK;
...
sub TIEHASH {
	...
	# if clone
	if (ref($time))
		{@{$self}{@BadNews::EzDate::ltimefields, 'epochsec'}=@{$time}}
...
sub setfromtime {
	...
	@{$self}{@BadNews::EzDate::ltimefields}=localtime($time);
	$self->{'year'} += 1900;
}
# 
# setfromtime
...
sub warn {
	my $self = shift;
	my $level = defined($self->{'warnings'}) ? $self->{'warnings'} : $BadNews::EzDate::default_warning;
...
sub STORE {
	...
	# ordinals
        ...
        # scope last month was dst outside the loop
        my $lastmonthwasdst;
		
		# loop until we get to this day in next or previous month
		while (1) {
			my ($newdaynum, $newmonthnum, $newmonthyear, $newmonthisdst);
			...
			($newdaynum, $newmonthnum, $newmonthyear, $newmonthisdst) = (localtime($cepoch))[3,4,5,8];

            $lastmonthwasdst = $newmonthisdst unless defined($lastmonthwasdst);

            #print "$cepoch\n";
            #print "$newdaynum, $newmonthnum, " . ($newmonthyear + 1900) . ": $newmonthisdst\n\n";

            # DST messes everything up here.. for some reason it defaults to 1 year prior
            # the following keywords are for my convenience .. as i've band-aided this problem
            # in next_month() by setting the year AFTER the monthnum.. lame
            # ERROR BUG PROBLEM UGLY CRAP FUCK

            # check to see if DST changed..
            unless ($lastmonthwasdst == $newmonthisdst) { 
                #print "$lastmonthwasdst, $newmonthisdst\n";
                if ($lastmonthwasdst == 1) {
                    # we were daylight savings time before.. and now we're not.. we sprang forward!
                    # so add 3600 (1 hr) to our lovely $cepoch and get the date again!
                    ($newdaynum, $newmonthnum) = (localtime($cepoch + 3600))[3,4];
                } else {
                    # we were not daylight savings time and now we are.. we fell back!
                    # so add 3600 (1 hr) to our lovely $cepoh and get the date again (this shouldnt matter but im
                    # being consistent)
                    ($newdaynum, $newmonthnum) = (localtime($cepoch - 3600))[3,4];
                }
            }

            # handle teh dst goodness
            $lastmonthwasdst = $newmonthisdst;
...
sub getepochday {
	...
	unless (defined $epoch_offset) {
		my %date;
		@date{@BadNews::EzDate::ltimefields} = localtime(0);
		$epoch_offset = 
...
BadNews::EzDate - Date and time manipulation made easy
...
BadNews::EzDate can be installed with the usual routine:
...
BadNews::EzDate was motivated by the simple fact that I hate dealing with date and time calculations,
...
=item Load the module and instantiate the object
 ...
 use BadNews::EzDate;
 my $mydate = BadNews::EzDate->new();  # the object defaults to the current date and time
...
with tomorrow's date:
  ...
  $date = BadNews::EzDate->new('tomorrow');
...
two EzDate objects, then determines if the first object is less than the second:
	...
	$mybday = BadNews::EzDate->new();
	$yourbday = BadNews::EzDate->new('tomorrow');
...
$BadNews::EzDate::overload to the name of the property to compare.  For example, the following code sets 
...
they are identical down to the second:
	...
	$date = BadNews::EzDate->new('January 3, 2001');

I<PLEASE NOTE>: $BadNews::EzDate::overload used to be named $BadNews::EzDate::compare.  I made a non-backwards
...
dates are on the same day of week by using their C<day of week> properties:
	...
	$date = BadNews::EzDate->new('January 3, 2001');
	$otherdate = BadNews::EzDate->new('January 10, 2001');
...
indicated by the C<$BadNews::EzDate::overload> variable). For example, to increment the day of the object, simply increment it
with C<++> like a number.  For example, the following code moves the day from Jan 31, 2003 to Feb 1, 2003:

  my $date = BadNews::EzDate->new('Jan 31, 2003');
...
The following are valid ways to instantiate an EzDate object:
 ...
 my $date = BadNews::EzDate->new();
 
 # a specific date and time
 my $date = BadNews::EzDate->new('Jan 31, 2001');
 
 # a date in DDMMMYYYY format
 my $date = BadNews::EzDate->new('14JAN2003');
 
 # a little forgiveness is built in (notice oddly place comma)
 my $date = BadNews::EzDate->new('14 January, 2003');
 
 # epoch second (23:27:39, Tue Apr 10, 2001 if you're curious) 
 my $date = BadNews::EzDate->new(986959659);
 
 # yesterday
 my $date = BadNews::EzDate->new('yesterday');
 
 # tomorrow
 my $date = BadNews::EzDate->new('tomorrow');
...
find it easier to create your own customer super-class of BadNews::EzDate so that you can 
set the custom formats in one place.  See "Super-classing BadNews::EzDate" below.
...
You can set the global default warning level by setting $BadNews::EzDate::default_warning.  For 
example, the following code sets the global default level to 2:

  $BadNews::EzDate::Tie::default_warning = 2;
...
You can also pass C<zero_hour_ampm> as an initial argument for C<new>:
  ...
  $date = BadNews::EzDate->new('January 31, 2002 1 am', zero_hour_ampm=>0);

=head2 after_create

C<after_create> is intended for use when you are super-classing EzDate.  By default,
C<after_create> does nothing.  See "Super-classing BadNews::EzDate" below for more details.
...
specific times.  So, for example, this string indicates July 25, 2003 and noon:
	...
	$mydate = BadNews::EzDate->new('July 23 2003 noon');
...
BadNews::EzDate and BadNews::EzDate::Tie.  For example, suppose you want to create a class
...
- Added feature that if BadNews::EzDate->new() is called with an unrecognized format, then
